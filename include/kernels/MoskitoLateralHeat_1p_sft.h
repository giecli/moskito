...
class MoskitoLateralHeat_1p_sft;

template <>
InputParameters validParams<MoskitoLateralHeat_1p_sft>();

class MoskitoLateralHeat_1p_sft : public Kernel
{
public:
  MoskitoLateralHeat_1p_sft(const InputParameters & parameters);
...
protected:
  ...
  // Radius tubing outer
  const MaterialProperty<Real> & _rti;
  // Thermal wellbore resistivity
  const MaterialProperty<Real> & _Uto;
  // Temperature at formation
  const MaterialProperty<Real> & _TRock;
  // Rock conductivity
  const MaterialProperty<Real> & _lambda;
  // Dimensionless Temperature
  const MaterialProperty<Real> & _TD;
