...
class MoskitoLatHeat_sft;

template <>
InputParameters validParams<MoskitoLatHeat_sft>();

class MoskitoLatHeat_sft : public Material, public NewtonIteration
{
public:
  MoskitoLatHeat_sft(const InputParameters & parameters);
...
protected:
  ...
  // Calculate Rayleigh Number needed for hc calculation
  Real dimensionlessTemperature();
  // Radius tubing outer
  MaterialProperty<Real> & _RadTubin;
