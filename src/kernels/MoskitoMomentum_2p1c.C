...
validParams<MoskitoMomentum_2p1c>()
{
  ...
  params.addClassDescription("Momentum conservation equation for 2 phase "
        "pipe flow and it returns volumetric flowrate.");
  return params;
}
...
MoskitoMomentum_2p1c::computeQpJacobian()
{
  ...
  j += 2.0 * _grad_rho[_qp] * _phi[_j][_qp] * _u[_qp];
  ...
  j += _dgamma_dhq[_qp] * _phi[_j][_qp] * _grad_h[_qp];
  j += _dgamma_dpq[_qp] * _phi[_j][_qp] * _grad_p[_qp];
  j += _dgamma_dq2[_qp] * _phi[_j][_qp] * _grad_u[_qp];
...
MoskitoMomentum_2p1c::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  {
    j += _grad_phi[_j][_qp] * _u[_qp] * _u[_qp];
    j += 2.0 * _phi[_j][_qp] * _u[_qp] * _grad_u[_qp];
    j += _well_sign[_qp] * _f[_qp] * _phi[_j][_qp] * _u[_qp]
          * _u[_qp] * _perimeter[_qp] * _well_dir[_qp] / (8.0 * _area[_qp]);
    j /= _area[_qp] * _area[_qp];
    // j += _dgamma_dph[_qp] * _phi[_j][_qp] * _grad_h[_qp];
    // j += _dgamma_dp2[_qp] * _phi[_j][_qp] * _grad_p[_qp];
    // j += _dgamma_dp[_qp] * _grad_phi[_j][_qp];
    // j += _dgamma_dpq[_qp] * _phi[_j][_qp] * _grad_u[_qp];
    // j += _grad_phi[_j][_qp];
  ...
  if (jvar == _h_var_number)
  {
    ...
    j += _dgamma_dph[_qp] * _phi[_j][_qp] * _grad_p[_qp];
    j += _dgamma_dhq[_qp] * _phi[_j][_qp] * _grad_u[_qp];
  }
