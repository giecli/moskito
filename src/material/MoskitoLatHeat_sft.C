...
#include "MoskitoLatHeat_sft.h"
#include "Conversion.h"

registerMooseObject("MoskitoApp", MoskitoLatHeat_sft);

template <>
InputParameters
validParams<MoskitoLatHeat_sft>()
{
    ...
    params.addRequiredParam<Real>("density_rock",
          "Density of the formation (kg/m³)");
    params.addRequiredParam<Real>("capacity_rock",
...
MoskitoLatHeat_sft::MoskitoLatHeat_sft(const InputParameters & parameters)
  : Material(parameters),
    NewtonIteration(parameters),
    _T(coupledValue("temperature")),
    _RadTubin(declareProperty<Real>("radius_tubbing_inner")),
    ...
    _Rock_conductivity(declareProperty<Real>("Rock_conductivity")),
    _Rock_alpha(getParam<Real>("thermal_diffusivity_rock")),
    _Rock_density(getParam<Real>("density_rock")),
    _Rock_capacity(getParam<Real>("capacity_rock")),
    _Uto(declareProperty<Real>("thermal_resistivity_well")),
    _Tdim(declareProperty<Real>("dimless_Temperature")),
...
Real
MoskitoLatHeat_sft::transienttimefunction(Real Uto)
{
...
Real
MoskitoLatHeat_sft::TemperatureFormation()
{
...
Real
MoskitoLatHeat_sft::TemperatureWBinterface(Real Uto, Real Temp)
{
  Real Twb = 0.0;
...
Real
MoskitoLatHeat_sft::TemperatureCasingAnnulusInterface(Real Uto, Real Temp)
{
  Real Tci = 0.0;
...
Real
MoskitoLatHeat_sft::TemperatureTubingOuter(Real Uto, Real Temp)
{
  Real Tto;
...
Real
MoskitoLatHeat_sft::RadialHeatTransferCoefficient(Real Uto, Real Temp)
{
  Real OverFtci = 0.0;
...
Real
MoskitoLatHeat_sft::ConvectiveHeatTransferCoefficient(Real Uto, Real Temp, Real grav)
{
  Real khc, hc;
...
Real
MoskitoLatHeat_sft::Grashof(Real Uto, Real Temp, Real grav)
{
  Real Gr;
...
Real
MoskitoLatHeat_sft::Rayleigh(Real grav, Real Uto, Real Temp)
{
  Real Lc, Ray;
...
Real
MoskitoLatHeat_sft::dimensionlessTemperature()
{
  // dimensionless time according to H&k 2010
  Real td;
  td = _Rock_lambda * Timing / (_Rock_density * _Rock_capacity * _well_assembly.back() * _well_assembly.back() / 4.0);
  Real Tdim;
  Tdim = std::log(std::pow(exp(1.0), -0.2 * td) + (1.5 - 0.3719 * std::pow(exp(1.0), -td)) * std::pow(td,0.5));
...
Real
MoskitoLatHeat_sft::computeReferenceResidual(const Real trail_value, const Real scalar)
{
  return 1e-2;
}

Real
MoskitoLatHeat_sft::computeResidual(const Real trail_value, const Real scalar)
{
  // Auxillary variable
  Real Aux, Uto;
...
Real
MoskitoLatHeat_sft::computeDerivative(const Real trail_value, const Real scalar)
{
...
void
MoskitoLatHeat_sft::computeQpProperties()
{
  _Rock_conductivity[_qp] = _Rock_lambda;

  _Tdim[_qp] = dimensionlessTemperature();
  ...
  _RadTubin[_qp] = _well_assembly[0] / 2.0;
...
Real
MoskitoLatHeat_sft::initialGuess(const Real trial_value)
{
  Real ini_guess;
  if (trial_value == 0.0)
